...
func _ready() -> void:
	...
	_add_stamp()
	_make_button("PlayAgain", "PLAY AGAIN", Vector2(274, 392), _play_again, true)
...
func _add_stamp() -> void:
	...
	var success_stamp_texture := load("res://assets/success_stamp.svg") as Texture2D
	if success_stamp_texture != null:
		var stamp := Sprite2D.new()
		stamp.texture = success_stamp_texture
...
func _make_button(button_name: String, label: String, position: Vector2, action: Callable, take_focus: bool) -> void:
	...
	button.add_theme_stylebox_override("focus", _button_style(LIME, CYAN))
...
func _draw() -> void:
	...
	draw_circle(Vector2(190, 130), 5, LIME)
	draw_circle(Vector2(760, 130), 5, CYAN)
	draw_circle(Vector2(220, 334), 4, PURPLE)
	draw_circle(Vector2(740, 334), 4, LIME)
	draw_string(_font, Vector2(267, 118), "TRANSMISSION COMPLETE", HORIZONTAL_ALIGNMENT_LEFT, -1, 30, INK)
	draw_string(_font, Vector2(303, 336), "You kept the relay station online.", HORIZONTAL_ALIGNMENT_LEFT, -1, 19, CYAN)
